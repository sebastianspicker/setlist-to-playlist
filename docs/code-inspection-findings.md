...
| DCI-007 | `apps/web/src/lib/musickit/token.ts`          | Non-null assertion `cachedToken!` after `isTokenValid()`                        | TypeScript does not narrow module-level variables across function calls. `isTokenValid()` guarantees `cachedToken !== null`, so the `!` is safe. | **Accepted:** `!` is safe; `isTokenValid()` checks `cachedToken !== null` before returning true.           |
