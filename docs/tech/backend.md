...
| `GET /api/apple/dev-token` | Apple Developer Token (JWT) for MusicKit. Returns `{ token: "..." }` or `{ error: "..." }`. |
| `GET /api/setlist/proxy?id=...` or `?url=...` | setlist.fm proxy. Returns setlist JSON or `{ error: "..." }`. API key stays server-side. |
