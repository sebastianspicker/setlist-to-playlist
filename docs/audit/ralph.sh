...
#
# Writes audit reports under ralph-audit/audit/ at repo root; logs and state in ralph-audit/.
...
if [[ "$SKIP_SECURITY" != "true" ]]; then
  ...
  # Setlist-to-playlist: Apple and setlist.fm secrets must not be in env when running autonomous agent
...
# Support both repo/ralph-audit/ (one level up = repo) and repo/.codex/ralph-audit/ (two levels up = repo)
